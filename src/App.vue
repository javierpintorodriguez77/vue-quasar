  ...
  <q-layout view="lHh Lpr lFf" class="bg-grey-2 text-body">
    ...
    <q-header elevated class="bg-primary text-white">
      <q-toolbar class="q-py-xs">
        ...
        <q-btn color="secondary" icon="add" label="Nuevo Servicio" @click="abrirModalCrear" unelevated rounded
            ...
            <q-card flat bordered :class="{
              ...
              <q-card-section class="q-pb-sm">
                <div class="row items-center no-wrap justify-between">
                  ...
                  <div class="col-auto">
                    <q-chip :color="obtenerColorEstadoEquipo(servicio.estadoEquipo)" text-color="white" size="md"
              ...
              <q-card-section class="q-py-sm text-body1 q-gutter-xs">
                <div><span class="text-weight-bold">Reparación:</span> {{ servicio.tipoReparacion || 'No especificada'
                  }}</div>
                ...
                <div><span class="text-weight-bold">Precio Total:</span> <span class="text-weight-bolder">${{
                    servicio.precio }}</span></div>
                ...
                <div v-if="servicio.estadoEquipo === 'entregado'"
                  class="q-mt-sm bg-grey-3 q-pa-sm rounded-borders text-center shadow-1">
                  <div class="text-weight-bold text-grey-9 text-caption">Calificación del cliente</div>
                  <q-rating v-model="servicio.calificacion" max="5" size="1.8em" color="amber-9" icon="star_border"
                    icon-selected="star" />
                </div>

                <div v-if="servicio.observaciones"
                  class="q-mt-xs text-italic text-grey-8 bg-grey-2 q-pa-xs rounded-borders">
              ...
              <q-card-actions align="right" class="bg-grey-1">
                <q-btn flat round color="primary" icon="edit" :disable="servicio.estadoEquipo === 'entregado'"
                  @click="abrirModalEditar(servicio, index)">
                  <q-tooltip>{{ servicio.estadoEquipo === 'entregado' ? 'Un servicio entregado no se puede editar' :
                    'Editar' }}</q-tooltip>
                </q-btn>
                <q-btn flat round color="negative" icon="delete" :disable="servicio.estadoEquipo === 'entregado'"
                  @click="confirmarEliminacion(index)">
                  <q-tooltip>{{ servicio.estadoEquipo === 'entregado' ? 'Un servicio entregado no se puede eliminar' :
                    'Eliminar' }}</q-tooltip>
                </q-btn>
              </q-card-actions>
            </q-card>
        ...
        <q-dialog v-model="modalAbierto" persistent transition-show="scale" transition-hide="scale">
          <q-card style="width: 550px; max-width: 95vw;" class="rounded-borders">
            <q-card-section class="row items-center bg-primary text-white q-py-sm">
              ...
              <div class="text-h6 text-weight-bold">{{ editandoIndex === null ? 'Registrar Servicio' : 'Editar Servicio'
                }}</div>
              <q-space />
            ...
            <q-form @submit.prevent="guardarServicio" class="q-pa-md q-gutter-sm text-body1">
              <q-input v-model="form.cliente" label="Nombre del cliente" outlined dense>
                <template #prepend><q-icon name="person" /></template>
              </q-input>

              <div class="row q-col-gutter-sm">
                <div class="col-6">
                  <q-select v-model="form.marca" :options="opcionesMarcas" label="Marca" outlined dense>
                    <template #prepend><q-icon name="branding_watermark" /></template>
                  </q-select>
                </div>
                <div class="col-6">
                  <q-input v-model="form.modelo" label="Modelo" hint="Ej: Galaxy A15, iPhone 13" outlined dense>
                    <template #prepend><q-icon name="smartphone" /></template>
                  </q-input>
                </div>
              </div>

              <q-select v-model="form.tipoReparacion" :options="opcionesReparacion" label="Tipo de reparación" outlined
                dense>
                <template #prepend><q-icon name="build" /></template>
              </q-select>

              <q-select v-model="form.tecnico" :options="opcionesTecnicos" label="Técnico que atendió" outlined dense>
                <template #prepend><q-icon name="badge" /></template>
              </q-select>

              <q-input v-model.number="form.precio" type="number" label="Precio cobrado" prefix="$" outlined dense>
                <template #prepend><q-icon name="payments" /></template>
              </q-input>

              <div class="row q-col-gutter-sm">
                <div class="col-6">
                  <q-select v-model="form.metodoPago" :options="opcionesMetodoPago" label="Método de pago" outlined
                    dense>
                    <template #prepend><q-icon name="account_balance_wallet" /></template>
                  </q-select>
                </div>
                <div class="col-6">
                  <q-select v-model="form.estadoPago" :options="opcionesEstadoPago" label="Estado del pago" outlined
                    dense>
              ...
              <q-input v-if="form.estadoPago === 'abono'" v-model.number="form.valorAbono" type="number"
                label="Monto abonado" prefix="$" outlined dense class="bg-orange-1 rounded-borders">
                <template #prepend><q-icon name="price_check" color="warning" /></template>
              </q-input>

              
              <q-select v-if="editandoIndex !== null" v-model="form.estadoEquipo" :options="opcionesEstadoEquipo"
                label="Estado del equipo" outlined dense>
                <template #prepend><q-icon name="sync" /></template>
              </q-select>

              <q-input v-model="form.observaciones" type="textarea" label="Observaciones (opcional)"
                hint="Ej: Pantalla partida, viene sin bandeja SIM..." outlined dense rows="2">
                <template #prepend><q-icon name="notes" /></template>
              </q-input>

              <q-card-actions align="right" class="q-mt-md q-px-none">
                <q-btn label="Cancelar" color="grey-7" flat v-close-popup />
                <q-btn :label="editandoIndex === null ? 'Guardar Servicio' : 'Actualizar Servicio'" type="submit"
                  color="primary" unelevated class="q-px-md" />
              </q-card-actions>
            </q-form>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="modalEliminarAbierto">
          <q-card class="rounded-borders">
            <q-card-section class="row items-center q-pb-none">
              ...
              <span class="q-ml-md text-subtitle1 text-weight-medium">¿Está seguro de que desea eliminar este
                registro?</span>
            </q-card-section>
